CREATE TABLE Instructor
(
	Email VARCHAR(255),
	FirstName VARCHAR(255),
	LastName VARCHAR(255),
	PRIMARY KEY (Email)
) ENGINE=InnoDB;

CREATE TABLE Course
(
	ID INT AUTO_INCREMENT NOT NULL,
	Dept VARCHAR(4) NOT NULL,
	CourseNumber INT NOT NULL,
	Title VARCHAR(255) NOT NULL,
	CreditHours INT NOT NULL,
	Description TEXT NOT NULL DEFAULT '',
	Structure TEXT NOT NULL DEFAULT '',
	PRIMARY KEY(ID),
	UNIQUE (Dept, CourseNumber),
	UNIQUE (Dept, Title)
) ENGINE=InnoDB;

CREATE TABLE CourseInstance
(
	ID INT AUTO_INCREMENT NOT NULL,
	CourseID INT NOT NULL,
	Instructor VARCHAR(255) NOT NULL,
	TermID INT(6) NOT NULL,
	State ENUM ('Sent', 'Approved', 'Ready', 'Finalized') NOT NULL DEFAULT 'Sent',
	CommentPrep TEXT NOT NULL DEFAULT '',
	CommentPrepActions TEXT NOT NULL DEFAULT '',
	CommentChanges TEXT NOT NULL DEFAULT '',
	CommentCLO TEXT NOT NULL DEFAULT '',
	CommentRecs TEXT NOT NULL DEFAULT '',
	PRIMARY KEY (ID),
	UNIQUE (CourseID, Instructor, TermID),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE,
	FOREIGN KEY (Instructor) REFERENCES Instructor (Email) ON UPDATE CASCADE
) ENGINE=InnoDB;

CREATE TABLE CLO
(
	ID INT AUTO_INCREMENT NOT NULL,
	CourseID INT,
	CLONumber INT,
	Description TEXT,
	PRIMARY KEY (ID),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE Outcomes
(
	ID INT AUTO_INCREMENT NOT NULL,
	Outcome CHAR(1) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
	Dept VARCHAR(4) NOT NULL,
	Description VARCHAR(255) NOT NULL,
	PRIMARY KEY (ID),
	UNIQUE (Outcome, Dept)
) ENGINE=InnoDB;

CREATE TABLE CLOOutcomes
(
	CLOID INT NOT NULL,
	OutcomeID INT NOT NULL,
	PRIMARY KEY (CLOID, OutcomeID),
	FOREIGN KEY (CLOID) REFERENCES CLO (ID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (OutcomeID) REFERENCES Outcomes (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE MasterCLO
(
	CLOID INT NOT NULL,
	CourseID INT NOT NULL,
	PRIMARY KEY (CLOID, CourseID),
	FOREIGN KEY (CLOID) REFERENCES CLO (ID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE CLOMetrics
(
	ID INT AUTO_INCREMENT NOT NULL,
	Mean INT,
	Median INT,
	High INT,
	Satisfactory INT,
	PRIMARY KEY (ID)
) ENGINE=InnoDB;

CREATE TABLE CLOAssessment
(
	CourseInstanceID INT NOT NULL,
	CLOID INT NOT NULL,
	MetricID INT NOT NULL,
	Method VARCHAR(255),
	UNIQUE (MetricID),
	FOREIGN KEY (CourseInstanceID) REFERENCES CourseInstance (ID) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (CLOID) REFERENCES CLO (ID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (MetricID) REFERENCES CLOMetrics (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE PerformanceCriteria
(
	OutcomeID INT NOT NULL,
	Criteria TEXT NOT NULL,
	FOREIGN KEY (OutcomeID) REFERENCES Outcome (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB

CREATE TABLE Prerequisites
(
	CourseID INT NOT NULL,
	PrerequisiteID INT NOT NULL,
	IsCorequisite BOOLEAN NOT NULL DEFAULT 0,
	PRIMARY KEY (CourseID, PrerequisiteID),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (PrerequisiteID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE PrerequisiteAlternatives
(
	CourseID INT NOT NULL,
	PrerequisiteID INT NOT NULL,
	AlternativeID INT NOT NULL,
	PRIMARY KEY (CourseID, PrerequisiteID, AlternativeID),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (PrerequisiteID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (AlternativeID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE TermsOffered
(
	CourseID INT NOT NULL,
	Summer BOOLEAN NOT NULL DEFAULT 0,
	Fall BOOLEAN NOT NULL DEFAULT 0,
	Winter BOOLEAN NOT NULL DEFAULT 0,
	Spring BOOLEAN NOT NULL DEFAULT 0,
	PRIMARY KEY (CourseID),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE TermState
(
	TermID INT(6) NOT NULL,
	State ENUM ('Sent', 'Approved', 'Ready', 'Finalized') NOT NULL DEFAULT 'Approved',
	PRIMARY KEY (TermID)
) ENGINE=InnoDB;

CREATE TABLE CourseContent
(
	ID INT NOT NULL AUTO_INCREMENT,
	CourseID INT NOT NULL,
	Content VARCHAR(255) NOT NULL DEFAULT '',
	PRIMARY KEY (ID),
	UNIQUE (CourseID, Content),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE LearningResources
(
	ID INT NOT NULL AUTO_INCREMENT,
	CourseID INT NOT NULL,
	Resource VARCHAR(255) NOT NULL DEFAULT '',
	PRIMARY KEY (ID),
	UNIQUE (CourseID, Resource),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE SyllabusTimestamp
(
	CourseID INT NOT NULL,
	LastRevision TIMESTAMP,
	PRIMARY KEY (CourseID),
	FOREIGN KEY (CourseID) REFERENCES Course (ID) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE DisabilitiesPolicy
(
	ID INT NOT NULL DEFAULT 0,
	Policy TEXT NOT NULL DEFAULT '',
	LastRevision TIMESTAMP,
	PRIMARY KEY (ID)
) ENGINE=InnoDB;

CREATE TABLE StudentConduct
(
	ID INT NOT NULL DEFAULT 0,
	URL VARCHAR(255) NOT NULL,
	LastRevision TIMESTAMP,
	PRIMARY KEY (ID)
) ENGINE=InnoDB;

CREATE TABLE CurrentTerm
(
	TermID INT(6) NOT NULL,
	PRIMARY KEY (TermID)
) ENGINE=InnoDB;
